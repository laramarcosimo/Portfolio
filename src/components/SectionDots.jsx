...
/** Paginación de la portada: un punto por sección, el activo en navy (como en la referencia). */
export default function SectionDots({ current, className = '' }) {
  return (
    <nav aria-label="Secciones de la portada" className={`flex items-center justify-center gap-0.5 ${className}`}>
      {homeSections.map((id) => (
