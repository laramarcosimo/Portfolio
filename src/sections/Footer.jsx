...
const linkClass = 'grid h-10 w-10 place-items-center rounded-full text-white transition hover:bg-white/10 hover:text-sky'
  ...
  return (
    <footer id="site-footer" className="relative z-10 bg-navy text-white">
      <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-5 py-10 sm:px-8 md:flex-row">
        <Logo variant="blanco" className="h-12" />
        ...
        <p className="text-sm font-medium text-white/80">© {new Date().getFullYear()} {site.name}. Todos los derechos reservados.</p>
      </div>
    </footer>
  )
}
